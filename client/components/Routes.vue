  ...
  <div id="mainRoutes">
    <div class="routeHolder">
      ...
      <nuxt-link
        class="indRoute"
        to="/portfolio"
        @click.native="selectedRoute('PORTFOLIO')"
      >PORTFOLIO</nuxt-link>
    ...
    <div class="currentRouteHolder" id="currentActiveRoute">
      <div class="routeCurrentText">{{ pageTitle }}</div>
    </div>
  </div>
</template>
  ...
  computed: {
    pageTitle() {
      if (this.currentRoute == "") {
        if (
          process.client &&
          document.getElementsByClassName("nuxt-link-exact-active")[0] != null
        ) {
          this.currentRoute = document.getElementsByClassName(
